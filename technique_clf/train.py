...
def main(val_fold, config):
    ...
    ds_train, ds_eval, ds_test, targets, tokenizer, dataset_distribution = load_datasets(config_data, val_fold)
    
    model = get_model(config_data, num_labels=len(targets))

    trainer = get_trainer(model, config_data, ds_train, ds_eval, targets, tokenizer, dataset_distribution)
